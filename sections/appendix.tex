...
Verifying condition \eqref{rule:c_sq0}.
    ...
    \end{equation}
Verifying condition \eqref{eq:csq1}.
    \begin{equation}
...
\section{Results for Section~\ref{sec:ring}}

\subsection{Rules for controlled diagrams}\label{sec:csound}
First, we verify the soundness of the axioms for controlled diagrams by plugging in basis states. Rules (\ref{rule:c_sq0}) and (\ref{rule:cstate0}) follow immediately from our definition of controlled matrices and states, respectively.
...
\subsection{Proofs for Section~\ref{sec:ring}}\label{sec:ringproofs}
...
\begin{proof}
    ...
    \end{equation}
    and likewise for $\phi_{n\plu 1}(q)$. We first verify correctness of the constructed controlled diagram
