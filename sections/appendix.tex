...
Before we prove Proposition \ref{prop:FF}, we first need to explain what the AND gate is in the ZXW-calculus.
...
\begin{definition}\label{lemma:and}
    \begin{equation}
...
The following proof implies that as the only place where $(\ref{rule:TA})$ applied is in the proof of the ZW-calculus rule $\left(\ref{eq:hopf}\right)$, it suffices to replace $(\ref{rule:TA})$ by $\left(\ref{eq:hopf}\right)$; all rules involving X spiders with two or more legs can be safely disregarded.
...
This section derives the axioms of Figure~\ref{fig:zxw_rules}, purely through syntactic equalities of axioms directly sourced from the arbitrary finite dimension setting derived in~\cite{poor2023completeness}, with the dimension then restricted to two to recover the qubit case.
...
The remainder of this section is to derive $(\ref{rule:HD})$ from $(EU)$ of~\cite{poor2023completeness}, for which we first derive a number of lemmas.
...
\begin{prop}
    ...
    \end{equation}
    where the middle of the three diagrams is the result of restricting the $(EU)$ axiom from~\cite{poor2023completeness} for arbitrary finite dimension, to the qubit (two-dimensional) setting.
\end{prop}
\begin{proof}
    \begin{equation*}
...
\begin{remark}
    As a useful observation, compare the following diagram for the Hadamard gate derived above, to the diagram for the AND gate from Lemma~\ref{lemma:and}.
    \begin{equation*}
...
The soundness of axioms $\ref{rule:c_sq0}$, $\ref{rule:c_sq1}$, $\ref{rule:cstate0}$, and $\ref{rule:cstate1}$ with respect to the interpretations in Equations~\eqref{eq:ctrlsqinterp} and~\eqref{eq:ctrlstateinterp}, immediately follows from applying the control states $\tikzfig{tikz/lemmas/x0} = \ket{0}$ and $\tikzfig{tikz/lemmas/x1} = \ket{1}$ respectively.
...
\begin{proof}
    ...
    Thus the inductive hypothesis states that:
    \begin{equation}
