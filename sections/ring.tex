...
Now we show that controlled matrix addition and multiplication satisfy the ring axioms. Associativity of $+, \times$ follow immediately from (\ref{rule:Aso}, \ref{rule:S1}), respectively. Commutativity of addition is just (\ref{rule:CMcom}). If $M_1 \circ M_2 \neq M_2 \circ M_1$, then the controlled matrices will of course not commute either, so we cannot expect commutativity of multiplication in general. The remaining ring properties are as follows:
