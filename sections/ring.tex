...
The only proofs we had to plug in basis states for are Lemmas~\ref{lem:csq_copy}, \ref{lem:csqaddcomm}, and \ref{lem:cscopy}. All other diagrammatic equalities in this paper were derivable from other rewrite rules. Therefore these, along with the definition of controlled states and square matrices (that when the control is $\ket{0}$ they result in $\ket{0...0}$ and the identity map respectively), form the only five Controlled Ring Rules in Figure~\ref{fig:zxw_rules}.

hold for any concrete realisations of $\tilde{M}$, for abstract $\tilde{M}$s they can only be verified via the plugging in of basis states.
...
Now we show that controlled matrix addition and multiplication satisfy the ring axioms. Associativity of $+, \times$ follow immediately from (\ref{rule:Aso}, \ref{rule:S1}), respectively. Commutativity of addition follows from the commutativity of matrix addition and Proposition \ref{prop:cmat_ops}, defining the following rewrite rule for controlled operators acting on mutually exclusive sectors:

\begin{lemma}\label{lem:csqaddcomm}
