\begin{figure}[htbp]
  ...
  The subset of ZX, ZW, and ZXW Rules with white background suffice for completeness of \emph{arithmetic diagrams} (Definition~\ref{def:arithmetic}), where \eqref{rule:TA} was used only to prove Lemma~\ref{lem:kill_quad}.
  Culminating in Theorem~\ref{thm:ctrl_pnf} of this work, we show that controlled states and controlled operators form rings. The arithmetic and Controlled Ring Rules, which are the above rules with white background, achieve completeness for all operations over these rings.}
  \label{fig:zxw_rules}
\end{figure}