...
Let $\tilde{E_n}$ be the set of controlled square matrices on $n$ qubits. The goal of this section is to prove that the addition and multiplication operations introduced above induce a ring on $\tilde{E_n}$. Likewise, we show that the set of controlled $n$-qubit states $\tilde{S_n}$ also forms a ring. Before doing so, we prove a few important lemmas. The first lemma enables us to copy controlled matrices. Note that all proofs can be found in the appendix.
...
This normal form is very closely related to the completeness normal form (see \cite{poor2023completeness}). The reason we introduce the definition of a PNF is that it is an arithmetic diagram and therefore has a more immediate arithmetic interpretation. The reason for the specific connectivity condition is that it enables a PNF to directly represent its own matrix.
